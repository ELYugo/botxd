...
client.on('group-participants-update', async (anu) => {
		...
		try {
			...
			if (anu.action == 'add') {
				...
				teks = `hola @${num.split('@')[0]}\ bienvenido a el grupo *${mdata.subject}* lea las reglas y comportese para no darle ban`
				...
				teks = `sayonara @${num.split('@')[0]} te extrañaremos o talvez no :v`
				let buff = await getBuffer(ppimg)
				client.sendMessage(mdata.id, buff, MessageType.image, {caption: teks, contextInfo: {"mentionedJid": [num]}})
			}
		} catch (e) {
	...
	client.on('message-new', async (mek) => {
		try {
			...
			mess = {
				wait: '❬❗❭ AGUARDE UM POUQUINHO (CASO DEMORA DEU ERRO) ❬❗❭',
				success: '️❬ ✔ ❭ PRONTINHO 🖤',
				error: {
